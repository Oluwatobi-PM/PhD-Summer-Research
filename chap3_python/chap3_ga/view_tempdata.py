"""Inspect an optimizer checkpoint saved under a case work folder."""
...
def optimizer_keys(data: np.lib.npyio.NpzFile) -> tuple[str, str, str] | None:
    ...
    for prefix in ("GA", "PSO", "DE", "MADS", "HYBRID"):
...
def tempdata_path(case_root: str | Path | None = None) -> Path:
    ...
    root = Path(case_root).resolve() if case_root is not None else Path.cwd().resolve()
    if root.name == "python_tempdata":
        return root / "tempdata.npz"
    return root / "work" / "python_tempdata" / "tempdata.npz"
...
def main(case_root: str | Path | None = None) -> None:
    target = tempdata_path(case_root)
