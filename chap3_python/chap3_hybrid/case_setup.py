"""Load case-level hybrid global-search + NOMAD/MADS setup files."""
...
def config_from_setup(path: str | Path) -> CaseConfig:
    """Build `CaseConfig` from a hybrid optimizer setup file."""
    ...
    max_simulations = getattr(module, "MAX_SIMULATIONS", getattr(module, "MAXGEN", None))
    if max_simulations is None:
        raise AttributeError("Hybrid setup requires MAX_SIMULATIONS or MAXGEN.")

    swarm_size = int(getattr(module, "SWARM_SIZE", getattr(module, "POPULATION_SIZE", 50)))
