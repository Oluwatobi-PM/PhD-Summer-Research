"""Load case-level DE setup files."""
...
def config_from_setup(path: str | Path) -> CaseConfig:
    """Build `CaseConfig` from a Differential Evolution setup file."""
    ...
    max_generations = getattr(module, "MAX_GENERATIONS", getattr(module, "MAX_ITERATIONS", getattr(module, "MAXGEN", None)))
    if max_generations is None:
        raise AttributeError("DE setup requires MAX_GENERATIONS or MAXGEN.")
    population_size = getattr(
        module,
        "DE_POPULATION_SIZE",
        getattr(module, "POPULATION_SIZE", getattr(module, "SWARM_SIZE", None)),
    )
    if population_size is None:
        raise AttributeError("DE setup requires DE_POPULATION_SIZE or POPULATION_SIZE.")
