"""Load case-level PSO setup files."""
...
def config_from_setup(path: str | Path) -> CaseConfig:
    """Build `CaseConfig` from a PSO setup file.

    The shared simulator/objective pipeline still uses `CaseConfig`; PSO only
    needs the common case fields plus swarm size and iteration count.
    """
    ...
    max_iterations = getattr(module, "MAX_ITERATIONS", getattr(module, "MAXGEN", None))
    if max_iterations is None:
        raise AttributeError("PSO setup requires MAX_ITERATIONS.")
    swarm_size = getattr(module, "SWARM_SIZE", getattr(module, "POPULATION_SIZE", None))
    if swarm_size is None:
        raise AttributeError("PSO setup requires SWARM_SIZE.")
